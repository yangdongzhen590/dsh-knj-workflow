...
/* ===== 流程运行面板：状态对比增强 + 运行中呼吸脉冲 =====
   只对「正在运行」这一主信号加动画（其余状态保持静态），脉冲柔和克制、契合 DSH 原生风格。
   色值为既有语义色（RUN_STATUS/STAGE_META），未新增调色板。 */
@keyframes knjRunPulse{0%,100%{opacity:.9}50%{opacity:.25}}
@keyframes knjRunBreath{0%,100%{box-shadow:0 0 0 3px rgba(47,107,255,.22)}50%{box-shadow:0 0 0 9px rgba(47,107,255,.05)}}
@keyframes knjBarStrip{to{background-position:14px 0}}
.knj-run-glow{animation:knjRunPulse 2s ease-in-out infinite}
.knj-node-dot.running{animation:knjRunBreath 1.6s ease-in-out infinite}
.knj-node-card.running{border-color:#2f6bff;background:var(--dsw-alias-bg-base)}
.knj-node-card.done{border-color:var(--dsw-alias-state-success-primary)}
.knj-node-card.pending{border-color:var(--dsw-alias-border-l2);opacity:.85}
.knj-node-card.failed{border-color:var(--dsw-alias-state-error-primary)}
.knj-node-card.skipped{opacity:.55}
.knj-progress.animated>div{background-image:linear-gradient(90deg,var(--dsw-alias-state-business-primary) 0 62%,rgba(47,107,255,.5) 62% 100%);background-size:14px 100%;animation:knjBarStrip .9s linear infinite}
`;
		...
		class SidebarTaskDetail extends react.Component {
			...
			runSvgNode(n, status, typeColor, zoom) {
				...
				if (n.type === 'gateway-xor' || n.type === 'gateway-and') {
					...
					kids.push(react.createElement('rect', { key: 'b', x: n.x - w / 2, y: n.y - hh / 2, width: w, height: hh, rx: 9, fill: n.type === 'human' ? '#f3efff' : (status === 'running' ? '#e7efff' : status === 'done' ? '#f0faf3' : 'var(--dsw-alias-bg-base)'), stroke: isSkipped ? '#b6bdc7' : (status === 'pending' ? 'var(--dsw-alias-border-l2)' : c), strokeWidth: status === 'running' ? 2.1 : 1, opacity: dim }));
					...
					// running 光环：贴合节点矩形的呼吸光边（非圆环；仅运行节点=当前主信号上动画）
					if (status === 'running') kids.push(react.createElement('rect', { key: 'halo', className: 'knj-run-glow', x: n.x - w / 2 - 5, y: n.y - hh / 2 - 5, width: w + 10, height: hh + 10, rx: 13, fill: 'rgba(47,107,255,.1)', stroke: '#2f6bff', strokeWidth: 2.2 }));
				}
				const isSel = sel;
			...
			renderNodeCard(s, i) {
				...
				const statusIcon = s.status === 'done' ? icon('check')
					...
					: (i + 1);
				return react.createElement('div', { key: s.id, className: 'knj-node-card' + (isRunning ? ' running' : s.status === 'done' ? ' done' : s.status === 'pending' ? ' pending' : s.status === 'failed' ? ' failed' : s.status === 'skipped' ? ' skipped' : '') },
					react.createElement('div', { className: 'knj-node-head', onClick: () => hasDetail && this.toggle(s.id) },
						react.createElement('div', { className: 'knj-node-dot' + (isRunning ? ' running' : ''), style: { background: meta.color } }, statusIcon),
			...
			render() {
				...
				return react.createElement('div', { style: { padding: 12 } },
					...
					// 整体进度条 + 摘要
					react.createElement('div', { className: 'knj-progress' + (task.status === 'running' ? ' animated' : ''), style: { width: '100%', marginTop: 12 } },
