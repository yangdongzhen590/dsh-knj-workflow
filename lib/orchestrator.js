...
// 本轮已放行过一次去向的 human（消费当前决策 / 重放历史决策都算放行）。
// 去向把流程带回同一个 human（如驳回 → fix → submit → 同一 human 的回环）时，再次到达
// = 新一轮审批，必须暂停等人工，不能再用 decidedMap 里的旧决策放行第二次——
// 否则 fix/submit 被反复重执行直到 maxRuns 超限报错，人工节点永远无法重新触发
// （task-mttp8q71/task-mttz5ok7 实测：驳回后 fix/submit 空转 → 任务卡 running/失败）。
const humanReleased = new Set();
...
function getCtx() {
  ...
  // taskId = task.id（流程实例唯一标识；同任务 resume/rerun 不变）——节点产出文件路径可用它拼接，
  // 让不同任务实例的产物互相隔离（等价写法 ${task.id}）。
  return {
    task: { id: task.id },
    taskId: task.id,
    inputs: args.inputs || {},
...
async function executeUntil(nodeId, joinId) {
  ...
  while (current && current !== 'end' && current !== joinId && safety < 1000) {
    ...
    if (node.type === 'task') {
      ...
      if (outSchema && outSchema.type) {
        // file/dir 是 UI 展示语义标记（值=路径字符串），不是 JSON Schema 类型——
        // 传给 subagent 前归一化为 string，避免 schema 校验失败
        const toAgent = { ...outSchema };
        if (toAgent.properties) {
          const props = {};
          for (const [k, v] of Object.entries(toAgent.properties)) {
            const t = v && v.type;
            props[k] = (t === 'file' || t === 'dir') ? { ...v, type: 'string' } : v;
          }
          toAgent.properties = props;
        }
        opts.schema = toAgent;
      }
      ...
      if (args.decision && args.decision.humanId === node.id) {
        ...
        humanReleased.add(node.id); // 本 walk 已放行过一次 → 回访即暂停（新一轮审批）
        current = route.to;
      } else if (decidedMap[node.id] && !humanReleased.has(node.id)) {
        // 历史决策重放：这个 human 已审批过（如顺序两个 human，后一个决策触发 resume 重走图）。
        // 直接走最后去向：目标已在 initialResults → skipSet 跳过；未执行过 → 正常执行。
        // 不注入意见（当初决策后的执行已带过意见）。
        // 每个 human 每轮 walk 只重放一次：去向若把流程带回同一 human（驳回回环），
        // 第二次到达意味着要重新审批，直接暂停而不是再次放行旧决策（见 humanReleased 注释）。
        const route = matchRoute(node, decidedMap[node.id]);
        humanReleased.add(node.id);
        current = route.to;
      } else {
  ...
  // 安全步数上限触发：current 仍是非终止节点 = 图存在未收敛的循环（如网关互指）。
  // 必须显式抛错（与下方「失败不吞错」教义一致）：静默 return ok 会让死循环任务被误标 success。
  if (current && current !== 'end' && current !== joinId) {
    throw new Error(`安全步数上限（1000）触发：工作流在 ${current} 处未收敛（疑似网关/边构成循环）`);
  }
  return { ok, current };
}
