...
test('orchestrator: 网关互指死循环触发安全上限时必须显式失败（不得静默 ok 返回）', async () => {
  // gw1 → gw2 → gw1 互指（default 边），不含 end：安全步数上限触发
  const wf = {
    id: 'wf', nodes: [
      { id: 'start', type: 'start' },
      { id: 'a', type: 'task', title: 'A', inputs: [], body: { prompt: 'A', mode: 'single', output: {} } },
      { id: 'gw1', type: 'gateway-xor', title: 'G1' },
      { id: 'gw2', type: 'gateway-xor', title: 'G2' },
    ],
    edges: [
      { from: 'start', to: 'a' },
      { from: 'a', to: 'gw1' },
      { from: 'gw1', to: 'gw2', default: true },
      { from: 'gw2', to: 'gw1', default: true },
    ],
  };
  const { result } = await runOrchestrator(wf, () => ({ ok: true }));
  assert.equal(result.ok, false, '死循环必须在安全上限处显式失败，静默 ok 会让任务假完成');
  assert.match(String(result.error || ''), /安全|上限|循环|limit|loop/i, '错误信息应说明是安全上限/循环');
});
...
test('orchestrator: task.id 可引用（${taskId} 与 ${task.id}），可拼产出路径隔离实例产物', async () => {
  const wf = {
    id: 'wf-taskid', nodes: [
      { id: 'start', type: 'start' },
      { id: 'a', type: 'task', title: 'A', inputs: [], body: { prompt: '产物目录：${cwd}/out/${taskId}/，别名：${task.id}', mode: 'single', output: {} } },
      { id: 'end', type: 'end' },
    ],
    edges: [{ from: 'start', to: 'a' }, { from: 'a', to: 'end' }],
  };
  ...
  const { result } = await runOrchestrator(wf, (label, prompt) => {
    ...
    return { ok: true };
  }, { task: { id: 'task-mt1uc64i', title: 'T', taskDir: '.tasks/task-mt1uc64i', cwd: 'D:/ws/proj' } });
  // 两种写法都解析为 task.id，拼接出的产出路径按实例隔离
  assert.ok(capturedPrompt.includes('产物目录：D:/ws/proj/out/task-mt1uc64i/'), capturedPrompt);
  assert.ok(capturedPrompt.includes('别名：task-mt1uc64i'), capturedPrompt);
  assert.ok(!capturedPrompt.includes('${task'), capturedPrompt);
  assert.equal(result.current, 'end');
});
...
test('orchestrator: file/dir 输出字段类型传给 subagent 前归一化为 string（避免 JSON Schema 校验失败）', async () => {
  const wf = {
    id: 'wf', nodes: [
      { id: 'start', type: 'start' },
      {
        id: 'a', type: 'task', title: 'A', inputs: [], body: {
          prompt: 'A', mode: 'single',
          output: { type: 'object', properties: { doc: { type: 'file', description: '文档路径' }, folder: { type: 'dir' }, title: { type: 'string' } } },
        },
      },
      { id: 'end', type: 'end' },
    ],
  ...
  const phaseCalls = [];
  const sandbox = {
    args: { config: wf, task: { id: 't1', title: 'T', taskDir: '.tasks/t1' } },
    agent: async (prompt, opts) => {
      agentCalls.push(opts?.label ?? '?');
      if (opts?.schema) capturedSchema = JSON.parse(JSON.stringify(opts.schema));
      return { doc: 'docs/设计.md', folder: 'docs', title: 'x' };
    },
    parallel: async (thunks) => Promise.all(thunks.map((t) => t())),
    phase: (title) => { phaseCalls.push(title); },
    log: () => {},
  ...
  assert.ok(capturedSchema, 'subagent 应收到输出 schema');
  assert.equal(capturedSchema.properties.doc.type, 'string', 'file 应归一化为 string');
  assert.equal(capturedSchema.properties.folder.type, 'string', 'dir 应归一化为 string');
  assert.equal(capturedSchema.properties.title.type, 'string', '普通 string 不受影响');
});
